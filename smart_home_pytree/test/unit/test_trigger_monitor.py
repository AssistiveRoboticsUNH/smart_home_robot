...
class TestTriggerMonitorCapabilities:
    ...
    def test_check_day_requirement(self, monitor):
        ...
        assert monitor.check_day_requirement(req, current_day="monday") is True # Case insensitive check
    ...
    def test_satisfied_protocols_priority(self, monitor):
        ...
        assert names.index("HighPrioProtocol.sub1") < names.index("LowPrioProtocol.sub1")
    ...
    def test_collect_wait_requests(self, monitor):
        ...
        req_data = {
            "ComplexSuccessProtocol.sub1": {"seconds": 10, "timestamp": now_ts}
        }
    ...
    def test_wait_resumed_by_time(self, monitor):
        ...
        assert name not in monitor.completed_protocols  # Should be removed so it can run again
    ...
    def test_check_success_on_conditions_met(self, monitor):
        ...
        monitor.pending_waits[name] = datetime.now() + timedelta(minutes=10) # Future time
        monitor.completed_protocols.add(name)
        
        # Manually register the success criteria
        monitor.monitor_state_success[name] = {
            "mode": "any",
            "conditions": [{"state": "button_a", "value": True}, {"state": "button_b", "value": True}]
        }
        
        # Mock Robot State: button_b is True
        def side_effect(key):
            if key == "button_b": return True
            return False
    ...
    def test_success_on_logic_all_vs_any(self, monitor):
        ...
        monitor.monitor_state_success[name_any] = {
            "mode": "any",
            "conditions": [{"state": "A", "value": True}, {"state": "B", "value": True}]
        }
        monitor.pending_waits[name_any] = datetime.now() + timedelta(seconds=100)
        
        # Robot State: A=False, B=True -> Should Succeed
        monitor.robot_interface.state.get.side_effect = lambda k: True if k == "B" else False
        monitor.check_success_on_conditions()
        ...
        monitor.robot_interface.state.get.side_effect = lambda k: True if k == "A" else False
        monitor.check_success_on_conditions()
    ...
    def test_mark_completed_instant_vs_periodic(self, monitor):
        ...
        monitor.protocols_yaml['protocols']['ManualTest'] = {
            'periodic_sub': {
                'high_level': {
                    'reset_pattern': {'type': 'periodic', 'minutes': 10}
                }
            },
            'instant_sub': {
                'high_level': {
                    'reset_pattern': {'type': 'instant'}
                }
            }
        }
    ...
    def test_wait_pending_cycle(self, monitor):
        ...
        bb.set("wait_requests", {name: {"seconds": 60, "timestamp": datetime.now().timestamp()}})
        monitor.collect_wait_requests()
        ...
        monitor.robot_interface.state.get.side_effect = lambda k: True if k == "button_a" else False
        monitor.check_success_on_conditions()
    ...
    def test_periodic_reset_logic(self, monitor):
        ...
        monitor.protocols_yaml['protocols']['PeriodicProtocol'] = {
            'sub1': {
                'high_level': {
                    'reset_pattern': {'type': 'periodic', 'minutes': 30}
                }
            },
            'instant_sub': {
                'high_level': {
