...
from smart_home_pytree.trees.guarded_move_to_person_location import (
    GuardedMoveToPersonLocationTree,
)
...
def test_move_to_person_success_diff_loc():
    ...
    move_tree = GuardedMoveToPersonLocationTree(
        node_name="move_to_person_test", robot_interface=robot_interface, num_attempts=3
    )
    move_tree.setup()
...
def test_guarded_move_aborts_when_person_moves():
    ...
    def on_moving_trigger(action_name):
        ...
        robot_interface.state.update(
            "person_location", "bedroom"
        )  # triggers guard to abort
    ...
    # === Assertions ===
    assert mock_nav_server.result_status == "canceled", (
        "Expected navigation to be cancelled"
    )
    assert mock_nav_server.triggered, "Expected navigation to start"
    assert move_tree.final_status == py_trees.common.Status.FAILURE, (
        "Tree should fail when guard condition breaks"
    )
