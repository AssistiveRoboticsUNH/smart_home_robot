...
def test_read_script_shouldnt_run_in_diff_location():
            ...
            for sub_key, sub_value in value.items():
                if sub_key in expected_keys_to_be_true:
                    assert sub_value is False, (
                        f"[FAILED] Blackboard key '{sub_key}' has value '{sub_value}', expected False"
                    )
    ...
    assert robot_final_loc != person_final_loc, (
        f"Expected robot and person not to be in same location, got robot={robot_final_loc}, person={person_final_loc}"
    )
