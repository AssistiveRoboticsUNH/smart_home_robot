...
def test_charge_robot_tree_success():
    ...
    runner_thread = threading.Thread(
        target=charge_robot_tree.run_until_done, daemon=True
    )
    runner_thread.start()
    ...
    assert mock_nav_server.complete_time and mock_dock_server.trigger_time, (
        "Missing timing info"
    )
    assert mock_nav_server.complete_time < mock_dock_server.trigger_time, (
        "Docking should start after navigation"
    )
...
def test_charge_robot_tree_charging_true():
    ...
    assert not mock_nav_server.triggered, (
        "Navigation should NOT be triggered if charging=True"
    )
    assert not mock_dock_server.triggered, (
        "Docking should NOT be triggered if charging=True"
    )
    assert final_status == py_trees.common.Status.SUCCESS, (
        f"Expected SUCCESS but got {final_status}"
    )
...
def test_charge_robot_tree_nav_fails():
    ...
    assert not mock_dock_server.triggered, (
        "Docking should not be triggered if navigation fails"
    )
    assert final_status == py_trees.common.Status.FAILURE, (
        f"Expected FAILURE but got {final_status}"
    )
